...
public enum Frame: Sendable, Hashable {
    ...
    /// Whether this frame's content is retransmitted when the packet carrying
    /// it is declared lost (RFC 9002 §13.3).
    ///
    /// ACK and PADDING carry no retransmittable state; PING carries none;
    /// DATAGRAM is unreliable by design; PATH_CHALLENGE/PATH_RESPONSE are
    /// regenerated by path validation, not retransmitted; CONNECTION_CLOSE is
    /// terminal. Everything else (CRYPTO, STREAM, RESET_STREAM, STOP_SENDING,
    /// flow-control, NEW_CONNECTION_ID, HANDSHAKE_DONE, …) carries state that
    /// MUST be re-sent in a new packet so the peer eventually receives it.
    @inlinable
    public var isRetransmittableOnLoss: Bool {
        switch self {
        case .padding, .ping, .ack, .connectionClose,
             .datagram, .pathChallenge, .pathResponse:
            return false
        default:
            return true
        }
